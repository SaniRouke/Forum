...
func (app *Application) handlerSignup(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodPost {
		...
		if !asciiRegex.MatchString(username) || !asciiRegex.MatchString(password) {
			utils.ErrorPage(w, http.StatusBadRequest, "My fellow skuf, your username and password can only contain ASCII characters between 33 and 125 \n If you're unfamiliar with the ASCII table, now is the time to check it out")
...
func (app *Application) handlerLogin(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodPost {
		...
		if err != nil {
			utils.ErrorPage(w, http.StatusUnauthorized, "Skuf Not Found")
			app.Log.Error(err.Error())
			return
		}
		...
		if err != nil {
			utils.ErrorPage(w, http.StatusUnauthorized, http.StatusText(http.StatusUnauthorized))
			app.Log.Error(err.Error())
			return
		}
